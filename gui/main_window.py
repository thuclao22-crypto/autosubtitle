...
class MainWindow(QMainWindow):
    ...
    def handle_export_process(self):
        """
        Hàm xử lý xuất video hoàn chỉnh: 
        ...
        if not input_path or not os.path.exists(input_path):
            print("LỖI: Đường dẫn đầu vào không hợp lệ!")
            return

        # Lấy dữ liệu hiện tại từ bảng phụ đề
        ...
        if not table_data:
            ...
            # Nếu là THƯ MỤC: Quét video đầu tiên để lấy mẫu lên bảng
            if os.path.isdir(input_path):
                ...
                print(f"Phát hiện Folder: Đang quét video đầu tiên làm mẫu...")
                self.process_scanning_flow(video_files[0])
            
            # Nếu là FILE đơn lẻ: Quét trực tiếp
            else:
                self.process_scanning_flow(input_path)
            
            print("--- Đã quét xong! Vui lòng duyệt nội dung trên bảng, chỉnh Style/Hiệu ứng rồi nhấn 'XUẤT VIDEO' lần nữa để Render. ---")
            return 

        # --- GIAI ĐOẠN 2: RENDER VIDEO (KHI BẢNG ĐÃ CÓ DỮ LIỆU) ---
        print("--- Bắt đầu Giai đoạn 2: Render video thành phẩm ---")
        
        # Thu thập cấu hình Style & Hiệu ứng từ giao diện
        style_config = {
            "font_size": self.font_size_spin.value(),
            "primary_color": self.current_color if hasattr(self, 'current_color') else "white", 
        ...
        if os.path.isdir(input_path):
            ...
            for v_path in video_files:
                ...
                print(f"\n>>> Đang Render hàng loạt: {filename}")
                self.run_final_render_flow(v_path, current_output, style_config)
            
            print("\n--- HOÀN THÀNH: ĐÃ XỬ LÝ XONG TOÀN BỘ FOLDER! ---")
        
        # Xử lý RENDER cho 1 FILE đơn lẻ
        else:
            filename = os.path.basename(input_path)
            out_name = f"subbed_{filename}"
            current_output = os.path.join(output_base, out_name) if output_base else os.path.join("outputs", out_name)
            
            self.run_final_render_flow(input_path, current_output, style_config)
            print(f"\n--- HOÀN THÀNH: Video đã được lưu tại {current_output} ---")
